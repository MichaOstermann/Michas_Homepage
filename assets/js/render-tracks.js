(function(){
	...
	async function render(){
		...
		// Prüfe ob wir auf einer Kategorie-Seite sind (party.html, rapp.html, etc.)
		const pageName = PAGE_PATH.split('/').pop() || '';
		const categoryMatch = pageName.match(/^(party|rapp|love|gemischt)\.html$/i);
		const category = categoryMatch ? categoryMatch[1].toLowerCase() : null;
		
		// Wenn Kategorie-Seite, lade direkt die Kategorie-JSON
		if (category) {
			const categoryJsonUrl = `${DIRNAME}/data/${category}.json`;
			try {
				const res = await fetch(categoryJsonUrl + cacheBuster, { cache: 'no-cache' });
				if (res.ok) {
					const categoryData = await res.json();
					// Wenn Kategorie-JSON geladen, verwende diese direkt
					if (categoryData && (Array.isArray(categoryData.tracks) || Array.isArray(categoryData))) {
						data = categoryData;
						console.log(`✅ Kategorie-JSON geladen: ${category}.json`);
					}
				}
			} catch (e) {
				console.warn(`⚠️ Kategorie-JSON nicht gefunden: ${categoryJsonUrl}`, e);
			}
		}

		// Fallback: Lade tracks.json (nur wenn keine Kategorie-JSON geladen wurde)
		if (!data) {
			for (let i = 0; i < TRACKS_URLS.length && !data; i++) {
				...
				try {
					...
					if (res.ok) {
						data = await res.json();
						console.log(`✅ tracks.json geladen: ${baseUrl}`);
						break;
					}
				} catch (_) {}
			}
		...
		// PHP-API nur als letzter Fallback (wird bei Live Server nicht funktionieren)
		if (!data && !category) {
		...
		// Filtere nach Kategorie, wenn wir auf einer Kategorie-Seite sind
		let filteredList = list;
		if (category) {
			filteredList = list.filter(track => {
				const trackCategory = (track.category || 'gemischt').toLowerCase().trim();
				return trackCategory === category;
			});
			console.log(`✅ ${filteredList.length} Tracks in Kategorie "${category}" gefunden`);
		}
		
		masterList = list; // Gesamte Liste für Suche behalten
		if (window.musicStats && typeof window.musicStats.update === 'function') {
		...
		if (initialQuery.trim() !== '') {
			...
			// Zeige gefilterte Liste auf Kategorie-Seiten, sonst alle
			renderTracks(category ? filteredList : masterList);
		}
