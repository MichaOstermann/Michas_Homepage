(function() {
  ...
  const isInTemplatesFolder = window.location.pathname.includes('/templates/');
  const DATA_URL = isInTemplatesFolder ? '../assets/content/templates.json?v=20251203' : 'assets/content/templates.json?v=20251203';
  let allTemplates = [];
  let isRootPage = !isInTemplatesFolder;
  let currentFilter = 'all';

  // DEBUG OUTPUT
  console.log('🔧 Templates Renderer gestartet');
  console.log('📍 Pathname:', window.location.pathname);
  console.log('📁 isInTemplatesFolder:', isInTemplatesFolder);
  ...
  function renderTemplateCard(template) {
    ...
    if (isComing) {
      ...
      // Preview-URL: Use relative path
      const previewPrefix = isRootPage ? 'templates/' : '';
      const previewURL = template.preview && template.preview !== '#' ? previewPrefix + template.preview : (isRootPage ? '/' : '../');
      actionsHTML = `
    ...
    const imagePrefix = isRootPage ? 'templates/' : '';
    const imageSrc = (template.previewImage || '').startsWith('/') 
      ? template.previewImage.substring(1)
      : imagePrefix + (template.previewImage || '');
      ...
      <article class="template-card" data-category="${template.category}" data-template-id="${template.id}">
        <div class="template-preview">
          ...
          <img src="${imageSrc}" alt="${template.title}" loading="lazy" 
            onerror="this.src='/assets/images/templates/placeholder-template.svg'"
  ...
  window.requestTemplate = function(slug, title) {
    ...
    document.getElementById('requestForm').addEventListener('submit', async function(e) {
      ...
      try {
        // Sende Anfrage an dedizierten Template-Endpoint
        const response = await fetch('/template-request.php', {
          method: 'POST',
          headers: {
        ...
        console.log(`✅ Templates geladen (normalisiert): ${allTemplates.length}`);
        ...
        if (result.success) {
          ...
          requests.push({
            ...
            status: result.mailFailed ? 'mail-failed' : 'sent'
          });
          ...
          modal.innerHTML = `
            <div style="background: linear-gradient(135deg, #0B0F16 0%, #1a1f2e 100%); 
                        padding: 3rem; 
                        border-radius: 20px; 
                        border: 2px solid ${result.mailFailed ? 'rgba(255,150,0,0.6)' : 'rgba(0,255,135,0.5)'};
                        max-width: 500px;
                        width: 90%;
                        text-align: center;
                        box-shadow: 0 20px 60px ${result.mailFailed ? 'rgba(255,150,0,0.35)' : 'rgba(0,255,135,0.3)'};">
              <div style="font-size: 4rem; margin-bottom: 1rem;">${result.mailFailed ? '⚠️' : '✅'}</div>
              <h2 style="color: ${result.mailFailed ? '#FFA500' : '#00FF87'}; margin-bottom: 1rem; font-size: 1.8rem;">
                Anfrage ${result.mailFailed ? 'gespeichert' : 'versendet!'}
              </h2>
              <p style="color: rgba(255,255,255,0.8); margin-bottom: 2rem; line-height: 1.6;">
                Deine Anfrage für <strong style="color: #FF6B00;">${title}</strong> wurde ${result.mailFailed ? 'lokal gespeichert (Mail-Versand fehlgeschlagen).' : 'erfolgreich an Michael gesendet.'}<br><br>
                ${result.mailFailed ? 'Aktuell konnte keine Mail zugestellt werden. Bitte ggf. direkt per Kontaktformular schreiben.' : 'Du erhältst eine Email mit dem Download-Link nach Freigabe.'}
              </p>
  ...
  window.rateTemplate = async function(templateId, stars, templateTitle) {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`⭐ Server Rating gespeichert: ${templateTitle} -> ${stars} Sterne (Neu Ø ${result.rating.average})`);
      } else {
    ...
    // DOM Live-Update für Karte
    const card = document.querySelector(`.template-card[data-template-id="${templateId}"]`);
    if (card) {
      const template = allTemplates.find(t => t.id === templateId);
      const ratingTextEl = card.querySelector('.template-rating-container .rating-text');
      const starsWrap = card.querySelector('.template-rating-stars');
      if (ratingTextEl && template) {
        const avg = template.rating || stars; // fallback
        const count = template.ratingCount || 1;
        ratingTextEl.textContent = `${avg.toFixed(1)} ( ${count} Bewertungen )`;
      }
      if (starsWrap) {
        starsWrap.querySelectorAll('.rating-star').forEach((starEl, idx) => {
          if (idx < (allTemplates.find(t => t.id === templateId).rating || stars)) {
  ...
  document.addEventListener('DOMContentLoaded', async function() {
    ...
    fetch(DATA_URL)
      ...
      .then(async data => {
        // Normalize incoming data shape to renderer expectations
        const src = Array.isArray(data) ? data : (data.templates || []);
        allTemplates = src.map(t => {
          const tech = t.tech || t.tags || t.features || [];
          return {
            id: t.id || t.slug || (t.name ? t.name.toLowerCase().replace(/\s+/g,'-') : 'template'),
            slug: t.slug || t.id || (t.name ? t.name.toLowerCase().replace(/\s+/g,'-') : 'template'),
            title: t.title || t.name || 'Unbenanntes Template',
            description: t.description || '',
            category: t.category || 'creative',
            // demo (page) lives under /templates/
            preview: t.preview && t.preview.endsWith('.html') ? t.preview : (t.demoUrl || t.preview || ''),
            // preview image
            previewImage: t.previewImage || t.previewImageUrl || (t.preview && t.preview.endsWith('.jpg') ? t.preview : '') || '',
